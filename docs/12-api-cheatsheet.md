...
Condensed lookup once you already understand the concepts from the earlier chapters. Not exhaustive — see https://tabulator.info/docs/6.3 for the full reference.
...
```js
// data
table.setData(arrayOrUrl)
table.replaceData(array)
table.updateData(array)
table.updateOrAddData(array)
table.addData(array, addToTop, index)
table.addRow(rowData, addToTop, index)
table.updateRow(rowId, newData)
table.deleteRow(rowIdOrComponent)
table.clearData()
table.getData()
table.getRow(rowId)
table.getRows()
...
table.clearFilter(includeHeaderFilters)
...
table.off(eventName, callback); // remove a listener
```
